...
def decode_from_latent_space(latent_points, grammar_model):
    ...
    return final_rnas, MFE_scores, g_c_scores, Whole_rnas, Whole_MFE_scores
...
while iteration < 15:
    ...
    valid_RNA_final, MFE_final, g_c_final, Whole_rnas, Whole_MFE_scores = decode_from_latent_space(next_inputs, grammar_model)
    ...
    targets = MFE_scores_normalized #+ g_c_scores_normalized # target_structure_scores_normalized # + logP_values_normalized + cycle_scores_normalized
    ...
    for i in range(len(valid_RNA_final)):
        if valid_RNA_final[ i ] is not None:
            ...
            score = current_MFE_scores_normalized #+current_g_c_scores_normalized # + current_target_structure_scores_normalized
        else:
            score = -max(y)[ 0 ]
    ...
    #save_object(Whole_rnas, "results/Whole_rnas{}.txt".format(iteration))
    #save_object(Whole_MFE_scores, "results/Whole_MFE_scores{}.txt".format(iteration))
