...
def decode_from_latent_space(latent_points, grammar_model):
    ...
    for i in range(latent_points.shape[ 0 ]):
        ...
        for j in range(decode_attempts):
            if ( ~np.equal(RNA_series[ j, i ], None)):
                ...
                Whole_length.append(len(RNA_series[ j, i ]))
                '''
                ...
                if (H_dist<min_H_dist):
                    ...
                    min_energy = energy
                '''
                len_R = len(RNA_series[ j, i ]) #length of the RNA sequence
    ...
    return final_rnas, MFE_scores, g_c_scores, length_scores, Whole_rnas, Whole_MFE_scores, Whole_gcs, Whole_gcs_scores, Whole_length
...
length_scores = np.loadtxt('../../latent_features_and_targets_grammar/length_scores_t.txt')
...
targets = MFE_scores_normalized + alpha*g_c_scores_normalized + betha*length_scores_normalized
...
while iteration < 10:
    ...
    valid_RNA_final, MFE_final, g_c_final, length_final, Whole_rnas, Whole_MFE_scores, Whole_gcs, Whole_gcs_scores, Whole_length = decode_from_latent_space(next_inputs, grammar_model)
    ...
    for i in range(len(valid_RNA_final)):
        if valid_RNA_final[ i ] is not None:
            ...
            current_length_scores = length_final[i]
            ...
            current_length_scores_normalized = (np.array(current_length_scores) - np.mean(length_scores)) / np.std(length_scores)

            #score = (current_SA_score_normalized + current_log_P_value_normalized + current_cycle_score_normalized)
            score = current_MFE_scores_normalized+alpha*current_g_c_scores_normalized+betha*current_length_scores_normalized # + current_target_structure_scores_normalized
        else:
            score = -max(y)[ 0 ]
    ...
    with open("results/Whole_length{}.txt".format(iteration), 'w') as txt_file:  # Specify encoding here
        for i in range(len(Whole_length)):
            txt_file.write(str(Whole_length[i]))
            txt_file.write('\n')
