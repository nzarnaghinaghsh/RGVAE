...
for i in range(len(RNA_data)):
    ...
    g_c_scores.append(abs(RNA_vae.g_c(tokens)-0.2))
...
np.savetxt('targets_t.txt', targets)
#np.savetxt('logP_values.txt', np.array(logP_values))
np.savetxt('MFE_scores_t.txt', np.array(MFE_scores))
np.savetxt('g_c_scores_t.txt', np.array(g_c_scores))
