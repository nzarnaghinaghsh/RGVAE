...
def decode_from_latent_space(latent_points, grammar_model):
    ...
    for i in range(latent_points.shape[ 0 ]):
        ...
        for j in range(decode_attempts):
            if ( ~np.equal(RNA_series[ j, i ], None)):
                ...
                m1 = "CGU"
                #m2 = "GG"
                if (rna_seq.count(m1)==0): # and rna_seq.count(m2)==0):
                    Whole_motif_scores.append(10)
                else:
                    Whole_motif_scores.append(0)
    
                #Compute forbidden:
                f1 = "AAU"
                f2 = "CGC"
                f3 = "UGC"
                if (rna_seq.count(f1)==0 and rna_seq.count(f2)==0 and rna_seq.count(f3)==0):
                    Whole_forbidden_scores.append(0)
                else:
                    Whole_forbidden_scores.append(10)
                '''
    ...
    return final_rnas, MFE_scores, g_c_scores, motif_scores, forbidden_scores, Whole_rnas, Whole_MFE_scores, Whole_gcs, Whole_gcs_scores, Whole_motif_scores, Whole_forbidden_scores
...
motif_scores = np.loadtxt('../../latent_features_and_targets_grammar/motif_scores_t.txt')
forbidden_scores = np.loadtxt('../../latent_features_and_targets_grammar/forbidden_scores_t.txt')
...
targets = MFE_scores_normalized + g_c_scores_normalized + motif_scores_normalized + forbidden_scores_normalized
y = -targets
...
while iteration < 10:
    ...
    valid_RNA_final, MFE_final, g_c_final, motif_final, forbidden_final, Whole_rnas, Whole_MFE_scores, Whole_gcs, Whole_gcs_scores, Whole_motif_scores, Whole_forbidden_scores = decode_from_latent_space(next_inputs, grammar_model)
    ...
    MFE_scores = np.loadtxt('../../latent_features_and_targets_grammar/MFE_scores_t.txt')
    g_c_scores = np.loadtxt('../../latent_features_and_targets_grammar/g_c_scores_t.txt')
    motif_scores = np.loadtxt('../../latent_features_and_targets_grammar/motif_scores.txt')
    forbidden_scores = np.loadtxt('../../latent_features_and_targets_grammar/forbidden_scores.txt')
    ...
    targets = MFE_scores_normalized + g_c_scores_normalized + motif_scores_normalized + forbidden_scores_normalized # target_structure_scores_normalized # + logP_values_normalized + cycle_scores_normalized
    '''
    #import sascorer
    ...
    for i in range(len(valid_RNA_final)):
        if valid_RNA_final[ i ] is not None:
            ...
            current_motif_scores = motif_final[i]
            current_forbidden_scores = forbidden_final[i]
            ...
            current_motif_scores_normalized = (np.array(current_motif_scores) - np.mean(motif_scores)) / np.std(motif_scores)
            current_forbidden_scores_normalized = (np.array(current_forbidden_scores) - np.mean(forbidden_scores)) / np.std(forbidden_scores)
            ...
            score = current_MFE_scores_normalized+current_g_c_scores_normalized + current_motif_scores_normalized + current_forbidden_scores_normalized # + current_target_structure_scores_normalized
        else:
            score = -max(y)[ 0 ]
    ...
    with open("results/Whole_motif_scores{}.txt".format(iteration), 'w') as txt_file:  # Specify encoding here
        for i in range(len(Whole_motif_scores)):
            txt_file.write(str(Whole_motif_scores[i]))
            txt_file.write('\n')

    with open("results/Whole_forbidden_scores{}.txt".format(iteration), 'w') as txt_file:  # Specify encoding here
        for i in range(len(Whole_forbidden_scores)):
            txt_file.write(str(Whole_forbidden_scores[i]))
            txt_file.write('\n')
